...
(function() {
  ...
  (async function prefetchNews() {
    try {
      ...
      if (cached && cached.length > 0) {
        _prefetchedNews = cached;
        return;
      }
      _prefetchedNews = await fetchAllRealNews();
      if (_prefetchedNews && _prefetchedNews.length > 0) {
        saveNewsCache(_prefetchedNews);
      }
    } catch (e) { /* silent */ }
  })();
  ...
  // --- 真实股票行情 (Yahoo Finance via CORS proxy) ---
  const STOCK_CACHE = {};
  ...
  function loadIndexCache() {
    try {
      ...
      if (Date.now() - cache.ts < 30000) return cache.data; // 30秒内直接用
    } catch(e) {}
    return null;
  }
  ...
  function startIndexRefresh() {
    ...
    // 后台抓取最新数据
    fetchIndexData();
    setInterval(fetchIndexData, 30000); // 30秒刷新
  }
  ...
  // 暴露到 window 以便 inline onclick 调用
  ...
  function drawKLineChart(fullKline, days) {
    ...
    const chartH = H - padding.top - padding.bottom - 70; // 上方K线区 + 下方成交量区
    const volH = 60;
  ...
  function filterNewsByLayer(layer) {
    ...
    $$('.news-card').forEach(card => {
      // 'all', 'news', 'overview', 'ranking' → 显示全部
  ...
  // 东方财富股票代码 → 标准ticker（用于K线查询）
  function emCodeToTicker(stockCode) {
    if (!stockCode || stockCode.length !== 6) return null;
    const code = stockCode.trim();
    if (code.startsWith('6')) return code + '.SS';  // 上海
    if (code.startsWith('0') || code.startsWith('3')) return code + '.SZ'; // 深圳
    if (code.startsWith('4') || code.startsWith('8')) return code + '.BJ'; // 北交所
    return code + '.SS';
  }
  ...
  function renderResearchCards(reports) {
    ...
    reports.forEach(r => {
      ...
      if (r.stockName && r.stockCode) {
        ...
        // 生成伪company对象用于renderStockBadge
  ...
  function init() {
    // 首屏关键渲染（每步单独保护）
    try { renderOverview(); } catch(e) { console.warn('renderOverview:', e.message); }
    try { renderDashboard(); } catch(e) { console.warn('renderDashboard:', e.message); }
    try { initSorting(); } catch(e) { console.warn('initSorting:', e.message); }
    try { initTheme(); } catch(e) {}
    try { initPageRouting(); } catch(e) { console.warn('initPageRouting:', e.message); }
    ...
    } catch(e) { console.warn('switchPage init:', e.message); }

    try { initUpdateTime(); } catch(e) {}
    try {
      const rt = $('#rankingTable');
      if (rt) { rt.dataset.sort = 'composite'; rt.dataset.asc = 'false'; }
    } catch(e) {}

    // 延迟渲染
    setTimeout(() => {
      try { renderCards(); } catch(e) { console.warn('renderCards:', e.message); }
      try { renderRanking(); } catch(e) { console.warn('renderRanking:', e.message); }
      try { startRealNewsFeed(); } catch(e) {}
      try { startIndexRefresh(); } catch(e) {}
      try { initSearch(); } catch(e) {}

      setTimeout(() => { try { runResearchCycle(); } catch(e) {} }, 1500);
      setTimeout(() => { try { refreshAllStocks(); } catch(e) {} }, 3000);
      setInterval(() => { try { refreshAllStocks(); } catch(e) {} }, 3 * 60 * 1000);
    }, 50);

    const totalCos = industryData.reduce((s, i) => s + i.companies.length, 0);
    const publicCos = industryData.reduce((s, i) =>
      s + i.companies.filter(c => c.ticker).length, 0);
    console.log(`🧠 AI 全产业链图谱 | 18 赛道 · ${totalCos} 家公司 · ${publicCos} 家可查看K线`);
  }
